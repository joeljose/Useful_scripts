...
usage() {
    ...
    echo "  No flags    Install tree"
    echo "  --uninstall Remove tree"
}

uninstall_tree() {
    if ! command -v tree &>/dev/null; then
        echo "tree is not installed. Nothing to uninstall."
        exit 0
    fi

    echo "This will remove tree."
    ...
    echo "Removing tree..."
    if ! sudo apt-get purge -y tree; then
        echo "Error: Failed to remove tree."
        exit 1
    fi
...
if command -v tree &>/dev/null; then
    echo "tree is already installed: $(tree --version)"
...
if ! sudo apt-get install -y tree; then
    echo "Error: Failed to install tree."
    exit 1
fi
